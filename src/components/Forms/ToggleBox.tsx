 ...
 */
export const ToggleBox = ({ checked, onChecked, children, color, ...props }: ToggleBoxProps) => {
  ...
  return (
    <div className="flex items-center space-x-2">
      ...
      />
      <span className={`w-10 h-5 rounded-xl relative border border-border-grey cursor-pointer ${checked ? `bg-${color ? color : 'lab-turquoise'}` : 'bg-black'}`}
