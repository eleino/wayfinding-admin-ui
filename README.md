...
- Add `base: "/admin/"` to vite.config.ts to make the app work when served from a subpath (e.g. /admin/). This is needed because the users' frontend is served from /, so the admin UI needs to be served from a subpath to avoid conflicts.